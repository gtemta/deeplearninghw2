...
def DNN(x,weights,biases,dropout):
    ...
    #DROP
    _x = tf.nn.dropout(x,dropout)
    
    fc1 = tf.add(tf.matmul(_x,weights['wd1']),biases['bd1'])
    fc1 = tf.nn.relu(fc1)
    # Fully connected layer2
    #DROP
    _fc1 = tf.nn.dropout(fc1,dropout)
    
    
    fc2 = tf.add(tf.matmul(_fc1,weights['wd2']),biases['bd2'])
    fc2 = tf.nn.relu(fc2)
    #Outputlayer  
...
# construct model
predict = DNN(x,weights,biases,keep_prob)
regulizer = 0
...
with tf.Session() as sess:
    ...
    while step * batch_size < iters:
        ...
        if step % display_step == 0:
            ...
            teloss, teacc = sess.run([cost, accuracy], feed_dict={x: mnist.test.images[:256],y: mnist.test.labels[:256], keep_prob: 1.})
